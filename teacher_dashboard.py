...
if notes_file:
    if notes_file.type == "application/pdf":
        ...
        img = img.convert('L').filter(ImageFilter.SHARPEN).filter(ImageFilter.SHARPEN).point(lambda x: 0 if x < 80 else 255, '1')
        text = pytesseract.image_to_string(img, config='--psm 7 -l eng --oem 1')
    else:
